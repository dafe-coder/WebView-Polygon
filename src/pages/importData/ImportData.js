...
import { setData, setCurrentAccount, setIsLogin } from '../../store/slices/storageSlice'
...
export const ImportData = () => {
	...
	const {restorePhraseValid, nameValid, name, registered} = useSelector(state => state.create)
	...
	return (
		<section className={cn('bg-white')}>
			...
			<div className='wallet_body'>
				<div>
					<Par color='primary'>Recover a wallet using your Secret Recovery Phrase.</Par>	
					<Input errorPar id='name' type='normal' label='Name' />
					<Textarea getAddress={getAddress} label='Recovery phrase / private key'/>
				</div>
			</div>
			<Button 
				className={disabledBtn && 'disabled'} 
				type='primary' 
				onClick={onAddNewAcc}>Import Wallet</Button>
		</section>
	)
}